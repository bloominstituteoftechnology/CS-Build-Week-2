...
class mapper:
    ...
    def create_starting_map(self):
        ...
        self.my_map = Graph()
        print(f"my_map: {self.my_map}")
        self.player = Player("DerekEtman", info_dict['room_id'])
        ...
        for m in path:
            ...
            for direction in exits:
                if self.my_map.vertices[room][direction] == m:
                    ...
                    print(
                        f"\nCurrent Room -> Title: {self.info['title']} ID: {self.info['room_id']} Items: {self.info['items']}")
                else:
                    continue
    ...
    def make_dash(self,direction,traversal):
        ...
        if response.status_code == 200:
            ...
            print('\ncooldown triggered - waiting 20 seconds. code =',response.status_code,response.content)
            time.sleep(20)
            self.get_info(what=what, direction=direction, backtrack=backtrack)
    ...
    def get_treasure(self):
        while True:
            ...
            if response['name'] == 'User 20676' and response['gold'] >= 1000:
                ...
                self.action('change_name', name='LORD_DEREK')
                time.sleep(self.wait)
                ...
                print(f"Got a name! Time to get a COIN. New Name: {response['name']}")
                time.sleep(self.wait)
                # self.action('status') #Check new name
            elif response['encumbrance'] <= response['strength'] - 2:
                ...
                print('Current Inventory: ', response['inventory'])
                time.sleep(self.wait)
            # Could potentially add a section to manage miner
                ...
                for item in response['inventory']:
                    ...
                    print(f"You're current gold: {response['gold']}")
    ...
    def get_coins(self):
      ...
      while coins < 1000:
        ...
        # Mine Coin
        print('Getting proof...')
        response = requests.post(f'https://lambda-treasure-hunt.herokuapp.com/api/bc/last_proof/', headers=self.header)
        new_proof = proof_of_work(response.get('proof'), response.get('difficulty'))
        time.sleep(self.wait)
        # Need to send new_proof in the mine request json
        response = requests.post(f'https://lambda-treasure-hunt.herokuapp.com/api/bc/mine/', headers=self.header, json={"proof":''})
